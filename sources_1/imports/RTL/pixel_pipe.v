...
module pixel_pipe(
    ...
    output out_vsync,
    output out_blank,
    output reg [7:0] tlp_dbg,      // last sampled top-left red value (pipe INPUT)
    // Toggles once per frame, in the SAME cycle tlp_dbg is updated. This is the
    // CDC handshake for carrying the TLP into the camera's frame header: tlp_dbg
    // is stable for a whole frame either side of the flip, so a destination that
    // 2FF-syncs this toggle and samples the bus on the edge can never tear.
    output reg       tlp_tog = 1'b0,
    ...
    always@(posedge clk) begin
        ...
        if((S==B)&&(N==O)) begin
            ...
            if (smp_dly != 4'd0) smp_dly <= smp_dly - 4'd1;
                ...
                smp_pend <= 1'b0;
                tlp_dbg  <= in_red;                     // capture the sampled TLP every frame
                tlp_tog  <= ~tlp_tog;                   // ...and flip the CDC handshake with it
