# =============================================================================
# pt_ftplus_merged.xdc -- Ft+ (FT601Q) pins and bus timing for the MERGED build.
#
# Read ALONGSIDE Au2_pt.xdc, which owns everything else including usb_tx/usb_rx.
# Derived from LauPythonCamera_Pt_Stack/ddr/pt_ft_plus_bottom_timed.xdc with the
# duplicate usb_tx PACKAGE_PIN dropped -- Au2_pt.xdc already assigns AA21, and
# both files agreed on it, so this is the only overlap in the whole merge.
#
# THE BUS TIMING IS KEPT FROM THE START, not added later. An earlier build of
# this interface streamed at full rate with zero dropped frames while silently
# corrupting ft_data[31:16]: with no set_output_delay the bus was never in the
# timing graph at all and Vivado reported a clean WNS. Constrain it before there
# is logic on it, so the constraints cannot be forgotten once there is.
# =============================================================================
...
# (usb_tx PACKAGE_PIN removed -- owned by Au2_pt.xdc)
